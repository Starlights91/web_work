...
<body>
	<div class="container">
		...
		<form action="signup.jsp" method="post" novalidate>
			...
			<!-- disabled : 3개의(userName & pwd & pwd2)상태값 관리해서 어느하나라도 false 이면, 버튼이 disabled 되도록함 -->
			<button class="btn btn-primary btn-sm" type="submit" disabled>가입</button>
		</form>
	</div>
	<script>
		// 아래 3개의 상태값 관리해서 위의 button 에 
		//아이디 유효성 여부를 관리할 변수
		let isIdValid=false;
		//비밀번호 유효성 여부를 관리할 변수
		...
		const checkForm = () => {
			...
			if(isIdValid && isPwdValid && isEmailValid){
				//전송 버튼에 disabled 속성을 제거(=removeAttribute)하고  -> "[type=submit]" 속성 선택자 type 속성의 값이 submit 인 요소 선택 <button type = "submit"> 
				document.querySelector("[type=submit]").removeAttribute("disabled");
			}else{
				//전송버튼에 disabled 속성을 추가(setAttribute)한다
		...
		//document.querySelector("#userName").addEventListener("input", (e)=>{
			...
			//fetch() 함수를 이용해서 get 방식으로 입력한 아이디를 보내고 사용가능 여부를 json 으로 응답받는다. (위에서 inputId valid 되어야 여기서부터 시작 가능)
		...
		*/	
		...
		const checkPwd = ()=>{
			...
			//비밀 번호를 검증할 정규 표현식(특수문자 포함여부) [a-zA-Z0-9] = [\W] 한글자영소문자, 대문자, 숫자포함.  <-> not [^\W]
			const reg_pwd=/[\W]/;
		...
		document.querySelector("#email").addEventListener("input", (e)=>{
			...
			//이메일을 검증할 정규 표현식  (@ 포함되어 있는지만 확인하는 )
			const reg_email=/@/;
