...
public class UserDao {
	...
	static {
		...
		dao=new UserDao(); //객체는 여기서 1번만 생성된다.
	}
	...
	public static UserDao getInstance() { //Dao 객체가 필요하면, UserDao dao=new UserDao() 호출이 불가해짐. => UserDao dao=new UserDao.getInstance(); 
	...
	//userName (지역변수)을 이용해서 회원 한명의 정보를 리턴하는 메소드 
	public UserDto getByUserName(String userName) {
		UserDto dto=null; //dto는 가입된 정보(rs)가 없으면 null, rs 가 있으면 하단 if 문 돌면서 각정보를 담는다.
