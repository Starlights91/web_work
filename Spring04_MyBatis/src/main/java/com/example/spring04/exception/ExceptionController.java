...
public class ExceptionController {
	...
	@ExceptionHandler(DataAccessException.class) // 처리하고 싶은 예외의 타입: (DataAccessException.class)
	public String dataAccess(DataAccessException dae, Model model) {
		model.addAttribute("title", "DB 관련 작업중에 예외가 발생했습니다."); //"title"키값으로  "DB 관련 작업중에 예외가 발생했습니다." 담는다.
		model.addAttribute("message", dae.getMessage()); //"message"키값으로 dae.getMessage() 담고.
		model.addAttribute("status", 500); //상태값 "status"키값으로 500 을 담고.
		
		// /WEB-INF/views/error/data-access.jsp 페이지에서 에러 정보를 응답한다.
		return "error/data-access"; //view page 로 error 정보를 포워드 할 수 있다.
	}
