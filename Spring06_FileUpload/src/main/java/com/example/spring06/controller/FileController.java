...
public class FileController {
	 ...
	 * 경로를 읽어와서 필드에 담는다.
	 */
	@Value("${file.location}") // Springframework 의 value (lombok 아님!)
	private String fileLocation; // 파일을 저장할 위치     #m  custom.properties에 있는 Users/minaoh/playground 경로를 참조.
	...
	public ResponseEntity<InputStreamResource> download(String orgFileName, String saveFileName, long fileSize) {
		 ...
		 * #m 응답은 파일데이터..로 String 이 아닌 ResponseEntity Generic은 <InputStreamResource>
		 */
	...
	public String fileUpload2(Model model, FileDto dto) { //input 요소의 type="file" name="myFile" 연관성 확인 하기
		
		// 매개 변수에 전달된 FileDto 객체에 폼 전송된 내용이 모두 들어 있다.
		String title = dto.getTitle();
		MultipartFile myFile = dto.getMyFile();
		
		// 파일 다운 받기 위해선 3개 필요: orgFileName , fileSize , saveFileName
		//원본 파일명 
	...
	public String fileUpload(String title, MultipartFile myFile, Model model) { //input 요소의 type="file" name="myFile" 연관성 확인 하기
