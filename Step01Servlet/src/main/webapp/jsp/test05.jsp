<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	//DB 에 저장되어 있었던 nick name 이라고 가정하자 //여기의 값들이 초기값으로 보임.
	String nickName="짱구";
	String gender="man"; // "man" or "woman" 이 DB에 저장되어 있다고 생각하면 된다. //여기에 들어간 값에 따라 하단의 input 요소(value)에 따라 checked 가 추가 된다.
	String job="student"; // "student" or "developer" or "etc" //여기에 들어간 값에 따라 하단의 option 에서 selected 가 추가 된다.
	//textarea 에 입력한 내용이 DB 에 저장되어 있었다고 가정
	String comment="좋은 하루 되세요 \n 어쩌구... \n 저쩌구... \n \t 들여쓰기";
	//DB 에 저장된 취미 정보라고 가정
	String hobbys="[\"game\", \"music\"]"; // ["game","music"] 초기에 선택되어져 보인다. -> 그래서 contains 에는 이 두개만 checked 가 입력 되어 있는거 볼 수 있다.
%>
<!DOCTYPE html>
<html>
<head>
	...
	<form action="update.jsp" method="get">
		...
		<div>
			...
			<select name="job" id="job">
				...
				<option value="etc">기타 <%=job.equals("etc") ? "selected" : ""%>></option>
			</select>
		</div>
		<div>
		...
		<button type="reset">취소</button> <!-- 이 버튼 누르면 초기값으로 렌더링되어돌아간다 모두 default 값으로  -->
	</form>
</body>
</html>